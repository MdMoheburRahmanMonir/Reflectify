...
export default function MyLessonsPage({ token }) {
    ...
    useEffect(() => {
        ...
        async function loadLessons() {
            try {
                ...
                const data = await GetUserLessons(userId,session, token);
                if (!canceled) {
                    setLessons(data || []);
                }
            } catch (error) {
    ...
    return (
        <>
            {lessons.length > 0
                ? <UserTableData lessons={lessons} session={session} token={token}/>
